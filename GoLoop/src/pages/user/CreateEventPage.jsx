...
function CreateEventPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!eventData.title || !eventData.location || !eventData.description || !eventData.locationDetail || !imageFile) {
    ...
    <div className="container mx-auto p-8 max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="p-6 bg-white rounded-lg shadow-md space-y-4">
        ...
        <div>
          ...
          <input type="text" name="locationDetail" id="locationDetail" value={eventData.locationDetail} onChange={handleChange} className="mt-1 block w-full p-2 border rounded-md" placeholder="Contoh: Gedung Serbaguna, Jl. Merdeka No. 10" required />
        </div>
        
        <div>
          <label htmlFor="description" className="block text-sm font-medium text-gray-700">Deskripsi Event</label>
          <textarea name="description" id="description" value={eventData.description} onChange={handleChange} rows="4" className="mt-1 block w-full p-2 border rounded-md" placeholder="Jelaskan tentang event Anda..." required />
        </div>
